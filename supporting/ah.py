...
class AHConnector:
    ...
    def get_anonymous_access_token():
        response = retry_request(
            ...
            headers=HEADERS, method='post', json={"clientId": "appie"}
        )
        if not response.ok:
    ...
    def search_products(self, query=None, page=0, size=750, sort='RELEVANCE', category=6401):
        response = retry_request('https://api.ah.nl/mobile-services/product/search/v2?sortOn=RELEVANCE',
            params={"sortOn": sort, "page": page, "size": size, "query": query,
                    "taxonomyId": category, "adType": "TAXONOMY"},
    ...
    def get_categories(self):
        print({**HEADERS, "Authorization": "Bearer {}".format(self._access_token.get('access_token'))})
        response = retry_request(
